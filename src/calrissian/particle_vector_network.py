...
class ParticleVectorNetwork(object):
    ...
    def cost_gradient(self, data_X, data_Y, thread_scale=1):
        ...
        for j in range(layer.output_size):
            ...
            d2 = dx**2 + dy**2 + dz**2
            exp_dij = np.exp(-d2)
            dot = prev_layer.nx * layer.nx[j] + prev_layer.ny * layer.ny[j] + prev_layer.nz * layer.nz[j]
            exp_dij *= dot
            ...
            # Vector gradient
            tmp = qj * dq / dot
            tx = tmp * (prev_layer.nx - dot * layer.nx[j])
            ty = tmp * (prev_layer.ny - dot * layer.ny[j])
            tz = tmp * (prev_layer.nz - dot * layer.nz[j])

            dc_dn_x[l][j] += np.sum(tx)
            dc_dn_y[l][j] += np.sum(ty)
            dc_dn_z[l][j] += np.sum(tz)

            tx = tmp * (layer.nx[j] - dot * prev_layer.nx)
            ty = tmp * (layer.ny[j] - dot * prev_layer.ny)
            tz = tmp * (layer.nz[j] - dot * prev_layer.nz)
        ...
        return dc_db, dc_dq, dc_dr, dc_dn
